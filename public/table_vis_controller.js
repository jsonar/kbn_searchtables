...
import { filterTableBySearch } from './table_filter.js';

// get the kibana/table_vis module, and make sure that it requires the "kibana" module if it
// didn't already
const module = uiModules.get('kibana/search_table_vis', ['kibana']);

// add a controller to tha module, which will transform the esResponse into a
// tabular format that we can pass to the table directive
module.controller('KbnSearchTableVisController', function ($scope) {
   ...
   */
  $scope.$watchMulti(['renderComplete', 'config.searchKeyword'], function ([, inputSearch]) {

    let tableGroups = $scope.tableGroups = null;
    let hasSomeRows = $scope.hasSomeRows = null;

    if ($scope.esResponse) {
      tableGroups = cloneDeep($scope.esResponse);
  ...
  // Update vis config when it changes in ui.
